...
import { getChallengeStatusCode } from "@/utils";
    ...
    try {
      ...
      if (res.data == "verifier") {
        ...
        setPendingReviews(
          res.data?.filter(
            (challenge) =>
              challenge.status == getChallengeStatusCode("ALLOTED") &&
              challenge.verifierId == verifierIdRes.data
          )
        );
        setRaisedChallenges(
          res.data?.filter(
            (challenge) =>
              challenge.status == getChallengeStatusCode("OPEN") &&
              challenge.verifierId != verifierIdRes.data
          )
        );
        setArchive(
          res.data?.filter(
            (challenge) =>
              challenge.status == getChallengeStatusCode("SUCCESSFUL") ||
              challenge.status == getChallengeStatusCode("REJECTED")
          )
        );
      }
    } catch (err) {
      ...
      <div>
        ...
        <div className="static my-8">
          {pendingReviews && pendingReviews.length > 0 ? (
            <CustomCarousel responsive={cardsResponsive}>
