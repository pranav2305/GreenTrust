...
import { getChallengeStatusCode, getStatusColor } from "@/utils";
...
export default function ChallengeCard({
  ...
  return (
    <div>
      <div
        className={`flex-none bg-white rounded-lg shadow-lg mr-6 p-4 w-full border-l-4 ${getStatusColor(
          challenge.status
        )}`}
      >
        <div className="flex flex-col justify-evenly py-2 px-2">
          {full && (
            <div className="flex justify-between mb-4 items-center">
              {stake && (
                <p className="text-xl font-bold whitespace-nowrap overflow-hidden text-ellipsis">
                  {stake.farm.name}
                </p>
              )}
              <Modal
                anchor={
                  <div className="right-4 top-4">
                    <IconButton icon={faInfo} styles="!bg-yellow !w-6 !h-6" />
                  </div>
                }
                popover={<CropDetailCard stake={stake} full={false} />}
              ></Modal>
            </div>
          )}
          <div
            ...
            <Modal
              ...
              popover={
                <SupportDocument
                  documents={
                    challenge.documents
                      ? JSON.parse(challenge.documents).proofs
                      : ""
                  }
                />
              }
